	...
	<div class="bootstrap_container">
            ...
            <div id="defaultmenu" class="navbar-collapse collapse">
                <ul class="nav navbar-nav">
					...
					<li class="active"><a href="trailers-now-showing.php"  class="dropdown-toggle">购票</b></a></li>
...
<!-- AddThis Smart Layers END -->
	...
	<?php
		$NAME_PURCHASE_PAGE = "";	$img_PURCHASE_PAGE = "" ;
		...
		$sql1 = "SELECT a.Movie_No, a.Name FROM Movie a ,Arrangement b WHERE a.Name = b.Arrange_Name AND datediff(b.Arrange_Date,curdate())>0 ";
		$sql2 = "SELECT a.Movie_No, a.Name,b.Arrange_Time FROM Movie a ,Arrangement b WHERE a.Name = b.Arrange_Name AND datediff(b.Arrange_Date,curdate()) = 0 ";
		$result1 = $conn->query($sql1);
		$result2 = $conn->query($sql2);
		$num_purchase_page1 = $result1->num_rows;
		$num_purchase_page2 = $result2->num_rows;
		
		function PurchasePageIMA (){
		
			$row = $GLOBALS['result2']->fetch_assoc();
			$GLOBALS['NAME_PURCHASE_PAGE'] =  $row["Name"];
			$GLOBALS['img_PURCHASE_PAGE'] =  $row["Movie_No"];
		}
		
		
		function PurchasePageASHITA ()
		{
...
EOT;
		...
		function outputPinfoIMA()
		{
			for($i = 0;$i < $GLOBALS['num_purchase_page2'];$i++){
				PurchasePageIMA();
				$a = $GLOBALS['NAME_PURCHASE_PAGE'] ;
				$b = $GLOBALS['img_PURCHASE_PAGE'];
					
				$output =<<<EOT
				<div class="col-md-3 trailers-now-showing-grid text-center">
				<img src="images/$b.jpg" alt="" />
				<a class="t-n-s-m" >$a</a>
				<a class="t-n-s-more" href="movie-select-show.php?new=$b">点击购买</a>
				</div>
EOT;
				echo $output;
			}
		...
		function outputPinfoASHITA()
		{
			for($i = 0;$i < $GLOBALS['num_purchase_page1'];$i++){
				PurchasePageASHITA();
				$a = $GLOBALS['NAME_PURCHASE_PAGE'] ;
				$b = $GLOBALS['img_PURCHASE_PAGE'];
				...
				<img src="images/$b.jpg" alt="" />
				<a class="t-n-s-m" >$a</a>
				<a class="t-n-s-more" href="movie-select-show.php?new=$b">点击预订</a>
				</div>
EOT;
				echo $output;
			}
