	...
	<div class="bootstrap_container">
            ...
            <div id="defaultmenu" class="navbar-collapse collapse">
                <ul class="nav navbar-nav">
                    ...
                    <li><a href="cinemas.php">电影</a></li>	  
					<li><a href="trailers-now-showing.php">购票</a></li>
					<li><a href="blog.php">评论</a></li>
...
<!-- AddThis Smart Layers END -->
	...
	<?php
		...
		$NAME_INNER_COMMENT_PAGE = $_GET['new'];
		if(is_numeric($NAME_INNER_COMMENT_PAGE)){
			$sql3 = "SELECT Name From Movie Where Movie.Movie_No = $NAME_INNER_COMMENT_PAGE";
			$result3 = $conn->query($sql3);
			$row = $GLOBALS['result3']->fetch_assoc();
			$GLOBALS['NAME_INNER_COMMENT_PAGE'] = $row["Name"];
		}
		else
			;

		$SS_INNER_COMMENT_PAGE = "";	
		$LS_INNER_COMMENT_PAGE = "" ;		$TYPE_INNER_COMMENT_PAGE = "";
		$Comment_INNER_COMMENT_PAGE = ""; $UsrName_INNER_COMMENT_PAGE = "";
		$NO_INNER_COMMENT_PAGE = "";
			
		$sql1 = "SELECT b.Comments,c.Usr_Username   FROM Comments b,Usr c WHERE '$NAME_INNER_COMMENT_PAGE' = b.Movie_Name AND b.UsrId = c.UsrId";
		$sql2 = "SELECT a.Movie_No,a.Short_synopsis,a.Long_synopsis,a.Movie_Type FROM Movie a WHERE a.Name = '$NAME_INNER_COMMENT_PAGE'";
		$result1 = $conn->query($sql1);
		$num_inner_comment_page = $result1->num_rows;
		$result2 = $conn->query($sql2);
			
		function InnerCommentPage ()
		{
			$row = $GLOBALS['result1']->fetch_assoc();
			$GLOBALS['Comment_INNER_COMMENT_PAGE'] = $row["Comments"];
			$GLOBALS['UsrName_INNER_COMMENT_PAGE'] = $row["Usr_Username"];

			
		}
		
		function outputInComminfo()
		{
			$row = $GLOBALS['result2']->fetch_assoc();
			$GLOBALS['SS_INNER_COMMENT_PAGE'] =  $row["Short_synopsis"];
			$GLOBALS['LS_INNER_COMMENT_PAGE'] =  $row["Long_synopsis"];
			$GLOBALS['TYPE_INNER_COMMENT_PAGE'] =  $row["Movie_Type"];
			$GLOBALS['NO_INNER_COMMENT_PAGE'] =  $row["Movie_No"];
			$_SESSION['movieid'] = $row["Movie_No"];
			
			
			$a = $GLOBALS['SS_INNER_COMMENT_PAGE'];
			$b = $GLOBALS['LS_INNER_COMMENT_PAGE'];
			$c = $GLOBALS['TYPE_INNER_COMMENT_PAGE'];
			$d = $GLOBALS['NAME_INNER_COMMENT_PAGE'];
			$g = $GLOBALS['NO_INNER_COMMENT_PAGE'];
			$output =<<<EOT
			<h3 class="post">$d</h3>
				<div class="last-article">
				<p class="artext"> $b</p>
					<h3>$a</h3>
					<iframe src="movie/$g.mp4" frameborder="0" allowfullscreen></iframe>
					<ul class="categories">
						<li><a href="#">$c</a></li>
					</ul>
					<div class="clearfix"></div>
...
EOT;
		...
		function outputInComments()
		{		
			$circle = $GLOBALS['num_inner_comment_page'];
			for($i = 0;$i < $circle;$i++){
				InnerCommentPage();
				$e = $GLOBALS['Comment_INNER_COMMENT_PAGE'];
				$f = $GLOBALS['UsrName_INNER_COMMENT_PAGE'];
...
?>
						...
						<textarea name="comment" form="comentform" type="text" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = '撰写评论';}" required="">撰写评论</textarea>
						<form action="addcomment.php" method="post" id="comentform">
						<input type="submit">
						</form>
				</div>
				<div class="clearfix"></div>
