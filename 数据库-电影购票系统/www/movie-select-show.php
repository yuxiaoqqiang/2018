...
$con->set_charset("utf8");
		...
		if(is_numeric($moviename)){
			$sql3 = "SELECT Name From Movie Where Movie.Movie_No = $moviename";
			...
			;
		$sql = "SELECT Movie_No,Name,Movie_Type,Director,Main_performer,Short_synopsis FROM movie where Name='".$moviename."'"; 
        ...
        if($result->num_rows>0) {
			...
			$Main_performer=$row['Main_performer'];
...
$sql2 = "SELECT Arrange_Date, Arrange_Time FROM arrangement b where Arrange_Name='".$moviename."' AND datediff(b.Arrange_Date,curdate()) >=0 order by Arrange_Date asc, Arrange_Time asc";
$con2 =mysqli_connect("localhost","zxrtt", "tongtong", "movie");
 ...
 $arr = array();
   if($result2->num_rows>0) {
            do{
				...
				$t = array();
				$t['date'] = $row['Arrange_Date'];
				$t['time'] = $row['Arrange_Time'];
				$arr[] = $t;
			}while($row);
...
function page($arr){
	...
	$tag[]=0;
	  for($i = 1; $i < $j; $i = $i + 1){
		 ...
		 for($k = 0; $k < count($tag) - 1; $k = $k + 1){
		      $head = 1;
		      for($i = $tag[$k]; $i < $tag[$k + 1]; $i = $i + 1){
			     if($head == 1){
				    $head = 0;
				 //填头部日期
				 $temp3 = $arr[$i];
				 $temp4 = $temp3['date']; $temp5 = $temp4; 
				 echo <<< EOF3
				 <div class="movie-date-selection">
				<ul>
					<li class="location">
						<i class="fa fa-map-marker">$temp4</i>
					</li>
EOF3;
				 ...
				 echo <<< EOF4
				 <li class="time">
						<a href="movie-payment.php?date=$temp5&timer=$temp4&moviename=$movname">$temp4</a>
				</li>
EOF4;
...
function page2($Movie_No,$Name, $MovieType,$Director,$Main_performer, $Short_synopsis){
	echo <<< EOF9
	
							<img src="images/$Movie_No$Movie_No$Movie_No.jpg" alt="" />
					...
					<div class="bahubali-details">
						...
						<p>类型：$MovieType</p>
						<p>导演：$Director</p>
						<p>主演：$Main_performer</p>
						<p>简介：$Short_synopsis</p>
 ...
 if(log == 1){
	...
	<div class="bootstrap_container">
            ...
            <div id="defaultmenu" class="navbar-collapse collapse">
                <ul class="nav navbar-nav">
					...
					<li class="active"><a href="trailers-now-showing.php"  class="dropdown-toggle">购票</a></li>
					<li class="dropdown w3_megamenu-fw"><a href="blog.php" class="dropdown-toggle">评论</a></li>
	...
	<div class="now-showing-list">
		<div class="col-md-4 movies-by-category movie-booking">
					...
					<div class="clearfix"></div>
					<?php page2($Movie_No,$Name, $MovieType,$Director,$Main_performer, $Short_synopsis); ?>
				</div>
			</div>
